...
namespace CarInsurance.Controllers
{
    public class InsureeController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "Id,FirstName,LastName,EmailAddress,DateOfBirth,CarYear,CarMake,CarModel,DUI,SpeedinTickets,CoverageType,Quote")] Insuree insuree)
        {
            decimal Quote = 50m;                                               //  This is the base payment
            int age = DateTime.Now.Year - insuree.DateOfBirth.Year;
            if (insuree.DateOfBirth > DateTime.Now.AddYears(-age)) age--;                      //  Customer's age calculation
            if (age <= 18)
            {
                Quote += 100m;
            }
            ...
            //  Ticket calculation
            Quote += 10 * insuree.SpeedinTickets;
            if (insuree.DUI == true)
            {
            ...
            {
                Quote += 0.5m;
                insuree.Quote = Quote;              //  Saves the value of calculated quote to the insuree.quote property
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,FirstName,LastName,EmailAddress,DateOfBirth,CarYear,CarMake,CarModel,DUI,SpeedinTickets,CoverageType,Quote")] Insuree insuree)
        {
            if (ModelState.IsValid)
            {
